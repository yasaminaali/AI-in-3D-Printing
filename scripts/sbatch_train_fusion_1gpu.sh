#!/bin/bash
#=============================================================
# SLURM Batch Script: Train FusionNet v2 — Single GPU
# For Phase 1 (smoke test) and Phase 2 (arch validation)
#=============================================================
#
# Prerequisites:
#   1. Run sbatch_build_fusion_data.sh first to create fusion_data.pt
#   2. Ensure sa_gpu_env is set up (bash scripts/setup_env.sh)
#
# Phase 1 (smoke test, 10 epochs, ~5 min):
#   sbatch scripts/sbatch_train_fusion_1gpu.sh
#
# Phase 2 (arch validation, 15 epochs, ~15 min):
#   EPOCHS=15 PATIENCE=15 sbatch scripts/sbatch_train_fusion_1gpu.sh
#
#SBATCH --job-name=fusion_p1
#SBATCH --output=fusion_train_1gpu_%j.out
#SBATCH --error=fusion_train_1gpu_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --gpus-per-node=h100:4
#SBATCH --mem=128G
#SBATCH --time=01:00:00
#SBATCH --account=aip-rnishat
#SBATCH --qos=normal
#=============================================================

echo "=============================================="
echo "  FusionNet v2 — Single GPU Training"
echo "  Job ID:    $SLURM_JOB_ID"
echo "  Node:      $SLURM_NODELIST"
echo "  GPUs:      $CUDA_VISIBLE_DEVICES"
echo "  Date:      $(date)"
echo "=============================================="

# --- Module loading ---
module load python/3.11.5 cuda/12.6

# --- Navigate to repo directory ---
cd "$SLURM_SUBMIT_DIR" || cd ~/links/projects/aip-rnishat/shared/AI-in-3D-Printing

echo "Working directory: $(pwd)"
echo ""

# --- Activate virtual environment ---
if [ ! -d "sa_gpu_env" ]; then
    echo "ERROR: Virtual environment not found. Run setup_env.sh first on the login node."
    exit 1
fi
source sa_gpu_env/bin/activate

echo "Python: $(which python3)"
echo "Python version: $(python3 --version)"
echo ""

# --- Check CUDA availability ---
python3 -c "
import torch
print(f'PyTorch version: {torch.__version__}')
print(f'CUDA available: {torch.cuda.is_available()}')
if torch.cuda.is_available():
    p = torch.cuda.get_device_properties(0)
    print(f'  cuda:0 - {p.name} ({p.total_memory / 1024**3:.0f} GB)')
"

if [ $? -ne 0 ]; then
    echo "ERROR: PyTorch/CUDA check failed."
    exit 1
fi

# --- Verify training data exists ---
DATA_PT="checkpoints/fusion_data.pt"
if [ ! -f "$DATA_PT" ]; then
    echo "ERROR: Training data not found at $DATA_PT"
    echo "Run sbatch_build_fusion_data.sh first."
    exit 1
fi
echo ""
echo "Training data: $DATA_PT ($(ls -lh "$DATA_PT" | awk '{print $5}'))"
echo ""

# --- Training configuration (Phase 1 defaults) ---
EPOCHS="${EPOCHS:-10}"
BATCH_SIZE="${BATCH_SIZE:-64}"
LR="${LR:-4e-4}"
PATIENCE="${PATIENCE:-10}"

echo "Configuration (Single GPU):"
echo "  Epochs:     $EPOCHS"
echo "  Batch size: $BATCH_SIZE"
echo "  LR:         $LR"
echo "  Patience:   $PATIENCE"
echo ""

# --- Build extra args ---
EXTRA_ARGS=""
if [ -n "$RESUME_CKPT" ]; then
    echo "Resuming from checkpoint: $RESUME_CKPT"
    EXTRA_ARGS="$EXTRA_ARGS --resume $RESUME_CKPT"
fi

echo "Starting single-GPU training..."
echo ""

# --- Launch directly (no torchrun = no DDP, single GPU) ---
PYTHONPATH="$(pwd)/src:$PYTHONPATH" python3 src/model/train_fusion.py \
    --epochs "$EPOCHS" \
    --batch_size "$BATCH_SIZE" \
    --learning_rate "$LR" \
    --patience "$PATIENCE" \
    $EXTRA_ARGS

EXIT_CODE=$?

echo ""
echo "=============================================="
echo "  Training finished at $(date)"
echo "  Exit code: $EXIT_CODE"
echo ""
echo "  Checkpoints saved to: checkpoints/"
ls -lht checkpoints/*.pt 2>/dev/null | head -5
echo "=============================================="

exit $EXIT_CODE
