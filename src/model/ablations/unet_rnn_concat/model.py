"""
U-Net + RNN (Concatenation) ablation model.

Architecture: Full 4-level ResU-Net + GRU history encoder + bottleneck
self-attention. RNN context is concatenated at the bottleneck instead
of using FiLM conditioning at every decoder level.

Tests: whether FiLM conditioning specifically matters vs simple
concatenation for integrating temporal context.
"""
...
from fusion_model import (
    ResBlock, DownBlock, UpBlock, BottleneckAttention, HistoryEncoder, NUM_ACTIONS
)


class UNetRNNConcat(nn.Module):
    """Full U-Net + GRU with concatenation fusion at bottleneck. No FiLM."""
    ...
    def forward(self, state, hist_actions, hist_py, hist_px, hist_cb, hist_ca, hist_mask):
        ...
        b = self.bottleneck_attn(b)  # [B, 4f, 8, 8]
        ...
        fused = torch.cat([b, ctx_spatial], dim=1)
        fused = self.fusion_proj(fused)  # [B, 4f, 8, 8]

        # Decoder with skip connections (no FiLM)
        u4 = self.dec4(fused, e4)
        u3 = self.dec3(u4, e3)
        u2 = self.dec2(u3, e2)
        u1 = self.dec1(u2, e1)
